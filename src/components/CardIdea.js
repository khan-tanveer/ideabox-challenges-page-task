import React from "react";
import "./CardIdea.css";
import Like from "../images/Like.svg";
import Dislike from "../images/Dislike.svg";
import Comment from "../images/Comment.svg";
import Share from "../images/Share.svg";
import Heart from "../images/Heart.svg";
import Calender from "../images/Calender.svg";
import MagicStick from "../images/MagicStick.svg";
import unpin from "../images/unpin.svg";
import edit from "../images/edit.svg";
import book from "../images/bookmark.svg";
import deelete from "../images/deelete.svg";

const CardIdea = () => {
  return (
    <div className="cardidea">
      <div className="cardidea__container">
        <p className="cardidea__idea">idea for</p>
        <div className="cardidea__header">
          <div className="cardidea__headerFirst">
            <h3>Challenge with its full capacity to test</h3>
          </div>
          <div className="cardidea__headerLast">
            <img className="cardidea__headerSvgs" src={unpin} alt="" />
            <img className="cardidea__headerSvgs" src={edit} alt="" />
            <img className="cardidea__headerSvgs" src={book} alt="" />
            <img className="cardidea__headerSvgs" src={deelete} alt="" />
          </div>
        </div>

        <div className="header__secoundLine">
          <img
            className="cardidea__avatar"
            src="data:image/png;base64,"
            alt=""
          />
          <p>Hello all</p>
          <ul>
            <li>Idea</li>
            <li>22 days ago</li>
            <li className="cardidea__secThirdChild">1 views</li>
          </ul>
        </div>

        <div className="cardidea__para">
          <p>
            This is the full description of the any particular challenge to test
            the challenge description will be overflow or not this is testing
            process.
          </p>
        </div>

        <div className="cardidea__tags">
          <p>tag1,tag2</p>
        </div>

        <img
          className="cardidea__imageSection"
          src="http://idea.cxdeployer.com/assets/MyOrgo/1_HZFTnCQNJBfGBdKcywcKXw.jpeg-1617792840343.jpeg-1619777973532.jpeg"
          alt=""
        />

        <div className="cardidea__viewSvg">
          <div className="cardidea__svgAndPara">
            {/* <img src={View} alt="" />
            <p>1</p> */}
          </div>

          <div className="cardidea__extimg">
            <button>
              <img className="cardidea__calmag" src={Calender} alt="" />
            </button>
            &nbsp;&nbsp;
            <button>
              <img className="cardidea__calmag" src={MagicStick} alt="" />
            </button>
          </div>
        </div>

        <div className="cardidea__likeComment">
          <div className="cardidea__likeCommentSectionButton">
            <img className="cardidea__likeCommentImage" src={Heart} alt="" />
            <p>3</p>
            <img className="cardidea__likeCommentImage" src={Like} alt="" />
            <p>0</p>
            <img className="cardidea__likeCommentImage" src={Dislike} alt="" />
            <p>0</p>
            <img className="cardidea__likeCommentImage" src={Comment} alt="" />
            <p>0</p>
            <img className="cardidea__likeCommentImage" src={Share} alt="" />
          </div>

          <div className="cardidea__likeCommButton">
            <button>
              <img className="cardidea__calmag" src={Calender} alt="" />
            </button>
            &nbsp;&nbsp;&nbsp;
            <button>
              <img className="cardidea__calmag" src={MagicStick} alt="" />
            </button>
          </div>
        </div>
      </div>
    </div>
  );
};

export default CardIdea;
