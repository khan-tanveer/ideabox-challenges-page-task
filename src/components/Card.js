import React from "react";
import "./Card.css";
import Like from "../images/Like.svg";
import Dislike from "../images/Dislike.svg";
import Bulb from "../images/Bulb.svg";
import Comment from "../images/Comment.svg";
import Share from "../images/Share.svg";
import View from "../images/View.svg";
import unpin from "../images/unpin.svg";
import edit from "../images/edit.svg";
import book from "../images/bookmark.svg";
import deelete from "../images/deelete.svg";
import reddot from "../images/reddot.svg";

const Card = () => {
  return (
    <div className="card">
      <div className="card__container">
        <div className="card__header">
          <div className="card__headerFirst">
            <h3>Challenge with its full capacity to test</h3>
            <p>
              <img src={reddot} alt="" />
              live
            </p>
            {/* <li>live</li> */}
          </div>
          <div className="card__headerLast">
            <img className="card__headerSvgs" src={unpin} alt="" />
            <img className="card__headerSvgs" src={edit} alt="" />
            <img className="card__headerSvgs" src={book} alt="" />
            <img className="card__headerSvgs" src={deelete} alt="" />
          </div>
        </div>

        <div className="header__secoundLine">
          <img
            className="card__avatar"
            src="data:image/png;base64,"
            alt=""
          />
          <p>Hello all</p>
          <ul>
            <li>Challenge</li>
            <li>22 days ago</li>
            <li className="card__secThirdChild">1 views</li>
          </ul>
        </div>

        <div className="card__para">
          <p>
            This is the full description of the any particular challenge to test
            the challenge description will be overflow or not this is testing
            process.
          </p>
        </div>

        <div className="card__tags">
          <p>tag1,tag2</p>
        </div>

        {/* <img
          className="card__imageSection"
          src="http://idea.cxdeployer.com/assets/MyOrgo/1_HZFTnCQNJBfGBdKcywcKXw.jpeg-1617792840343.jpeg-1619777973532.jpeg"
          alt=""
        /> */}

        <div className="card__viewSvg">
          <div className="card__svgAndPara">
            <img src={View} alt="" />
            &nbsp;&nbsp;&nbsp;
            <p>1</p>
          </div>
          <button>Post an idea</button>
        </div>

        <div className="card__likeComment">
          <div className="card__likeCommentSectionButton">
            <img className="card__likeCommentImage" src={Bulb} alt="" />
            <p>3</p>
            <img className="card__likeCommentImage" src={Like} alt="" />
            <p>0</p>
            <img className="card__likeCommentImage" src={Dislike} alt="" />
            <p>0</p>
            <img className="card__likeCommentImage" src={Comment} alt="" />
            <p>0</p>
            <img className="card__likeCommentImage" src={Share} alt="" />
          </div>

          <div className="card__likeCommButton">
            <button>Post an idea</button>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Card;
